import { useState, useEffect } from "react";

const STORES = [
  { id: "SMR", name: "Santa María La Ribera", lat: 19.44815114965266, lng: -99.15418204004736 },
  { id: "TAB", name: "Tabacalera", lat: 19.43910242690003, lng: -99.15578818924966 },
  { id: "JUA", name: "Juárez", lat: 19.426522070418077, lng: -99.15542057575757 },
  { id: "CEN", name: "Centro", lat: 19.435325031610592, lng: -99.13280633342903 },
  { id: "JAR", name: "Jardín", lat: 19.45033734600347, lng: -99.16094680274179 },
  { id: "DVA", name: "Del Valle", lat: 19.373600044458726, lng: -99.16078647575902 },
];

const RADIUS_METERS = 300;
const LATE_MINUTES = 15;
const ADMIN_EMAILS = ["luiscgarinian@me.com", "ferhughes04@gmail.com", "saiv1221hughes@gmail.com"];

const SCHEDULES = {
  matutino: {
    label: "Matutino",
    getTime: (day) => {
      if (day === 6) return { h: 7, m: 30 };
      if (day === 0) return { h: 8, m: 30 };
      return { h: 7, m: 0 };
    },
  },
  intermedio: {
    label: "Intermedio",
    getTime: () => ({ h: 10, m: 0 }),
    windowEnd: { h: 11, m: 0 },
    adminNote: true,
  },
  vespertino: {
    label: "Vespertino",
    getTime: (day) => {
      if (day === 0 || day === 6) return { h: 15, m: 0 };
      return { h: 14, m: 0 };
    },
  },
};

const POSITIONS = ["Auxiliar", "Supervisor", "Administración"];

function haversine(lat1, lon1, lat2, lon2) {
  const R = 6371000;
  const dLat = ((lat2 - lat1) * Math.PI) / 180;
  const dLon = ((lon2 - lon1) * Math.PI) / 180;
  const a =
    Math.sin(dLat / 2) ** 2 +
    Math.cos((lat1 * Math.PI) / 180) *
      Math.cos((lat2 * Math.PI) / 180) *
      Math.sin(dLon / 2) ** 2;
  return R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
}

function generateEmployeeId(storeId, lastName, position) {
  const storeChar = storeId[0].toUpperCase();
  const lastTwo = lastName.replace(/\s/g, "").substring(0, 2).toUpperCase();
  const rankMap = { Auxiliar: "2", Supervisor: "1", Administración: "3" };
  const rank = rankMap[position] || "2";
  const rand = Math.floor(Math.random() * 100).toString().padStart(2, "0");
  return `${storeChar}${lastTwo}${rank}${rand}`;
}

function formatMin(mins) {
  if (!mins || mins === 0) return "0 min";
  if (mins < 60) return `${mins} min`;
  const h = Math.floor(mins / 60);
  const m = mins % 60;
  return m > 0 ? `${h}h ${m}min` : `${h}h`;
}

function getCurrentPeriod() {
  const now = new Date();
  const day = now.getDay();
  const totalMin = now.getHours() * 60 + now.getMinutes();
  for (const [key, sched] of Object.entries(SCHEDULES)) {
    const { h: sh, m: sm } = sched.getTime(day);
    const schedMin = sh * 60 + sm;
    if (totalMin >= schedMin - 30 && totalMin <= schedMin + 120) {
      return { key, sched, scheduledMin: schedMin };
    }
  }
  return null;
}

function getStorage(key, fallback) {
  try { return JSON.parse(localStorage.getItem(key)) ?? fallback; }
  catch { return fallback; }
}
function setStorage(key, val) {
  localStorage.setItem(key, JSON.stringify(val));
}

const p = {
  coffee: "#3B1F0E", cream: "#F5EDD7", caramel: "#C8862A",
  espresso: "#1A0A02", milk: "#FDF8EE", foam: "#EDE0C4",
  green: "#2E7D32", red: "#C62828", amber: "#E65100", gray: "#6D6D6D",
};

const css = `
  @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;700&family=DM+Sans:wght@300;400;500&display=swap');
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  html, body, #root { height: 100%; }
  body { font-family: 'DM Sans', sans-serif; background: ${p.milk}; color: ${p.espresso}; }
  .app { max-width: 430px; margin: 0 auto; min-height: 100vh; display: flex; flex-direction: column; position: relative; }
  .header { background: ${p.coffee}; color: ${p.cream}; padding: 20px 20px 16px; text-align: center; flex-shrink: 0; }
  .header h1 { font-family: 'Playfair Display', serif; font-size: 22px; letter-spacing: 0.5px; }
  .header p { font-size: 12px; color: ${p.foam}; margin-top: 3px; opacity: 0.8; }
  .content { flex: 1; padding: 18px 16px 80px; overflow-y: auto; }
  .card { background: white; border-radius: 14px; padding: 18px; margin-bottom: 14px; box-shadow: 0 2px 12px rgba(59,31,14,0.07); }
  .card-title { font-family: 'Playfair Display', serif; font-size: 17px; color: ${p.coffee}; margin-bottom: 14px; }
  label { font-size: 12px; font-weight: 500; color: ${p.gray}; display: block; margin-bottom: 5px; text-transform: uppercase; letter-spacing: 0.5px; }
  select, input[type=text], input[type=email] {
    width: 100%; padding: 11px 13px; border: 1.5px solid ${p.foam}; border-radius: 10px;
    font-family: 'DM Sans', sans-serif; font-size: 15px; background: ${p.milk};
    color: ${p.espresso}; margin-bottom: 13px; outline: none; transition: border 0.2s; appearance: none;
  }
  select:focus, input:focus { border-color: ${p.caramel}; }
  .btn { width: 100%; padding: 13px; border: none; border-radius: 12px; font-family: 'DM Sans', sans-serif; font-size: 15px; font-weight: 500; cursor: pointer; transition: all 0.15s; }
  .btn-primary { background: ${p.coffee}; color: ${p.cream}; }
  .btn-primary:hover { background: ${p.espresso}; }
  .btn-primary:disabled { background: #ccc; cursor: not-allowed; }
  .btn-secondary { background: ${p.foam}; color: ${p.coffee}; margin-top: 10px; }
  .btn-check { background: ${p.caramel}; color: white; font-size: 16px; padding: 16px; }
  .btn-check:disabled { background: #ccc; cursor: not-allowed; }
  .badge { display: inline-block; padding: 3px 9px; border-radius: 20px; font-size: 12px; font-weight: 500; }
  .badge-green { background: #E8F5E9; color: ${p.green}; }
  .badge-red { background: #FFEBEE; color: ${p.red}; }
  .badge-amber { background: #FFF3E0; color: ${p.amber}; }
  .badge-gray { background: ${p.foam}; color: ${p.gray}; }
  .stat-row { display: flex; justify-content: space-between; align-items: center; padding: 9px 0; border-bottom: 1px solid ${p.foam}; }
  .stat-row:last-child { border-bottom: none; }
  .stat-label { font-size: 13px; color: ${p.gray}; }
  .stat-value { font-size: 14px; font-weight: 500; }
  .nav { position: fixed; bottom: 0; left: 50%; transform: translateX(-50%); width: 100%; max-width: 430px; display: flex; background: ${p.coffee}; z-index: 100; }
  .nav-btn { flex: 1; padding: 12px 4px; border: none; background: transparent; color: ${p.foam}; font-size: 11px; font-family: 'DM Sans', sans-serif; cursor: pointer; display: flex; flex-direction: column; align-items: center; gap: 2px; transition: background 0.15s; }
  .nav-btn.active { background: rgba(255,255,255,0.13); color: white; }
  .nav-icon { font-size: 17px; }
  .info-box { border-left: 4px solid; border-radius: 0 10px 10px 0; padding: 11px 13px; margin-bottom: 13px; font-size: 13px; line-height: 1.6; }
  .info-green { background: #E8F5E9; border-color: ${p.green}; color: #1B5E20; }
  .info-amber { background: #FFF8E1; border-color: ${p.amber}; color: #5D4037; }
  .info-red { background: #FFEBEE; border-color: ${p.red}; color: #B71C1C; }
  .rec-row { padding: 10px 0; border-bottom: 1px solid ${p.foam}; }
  .rec-row:last-child { border-bottom: none; }
  .rec-name { font-weight: 500; font-size: 14px; }
  .rec-meta { font-size: 12px; color: ${p.gray}; margin-top: 2px; }
  .id-badge { font-family: monospace; font-size: 20px; letter-spacing: 3px; background: ${p.foam}; padding: 12px 20px; border-radius: 10px; text-align: center; color: ${p.coffee}; font-weight: 700; }
  .dot { width: 8px; height: 8px; border-radius: 50%; display: inline-block; margin-right: 6px; flex-shrink: 0; }
  .mini-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-bottom: 12px; }
  .mini-card { background: ${p.milk}; border-radius: 8px; padding: 8px 6px; text-align: center; }
  .mini-val { font-size: 20px; font-weight: 500; color: ${p.coffee}; }
  .mini-lbl { font-size: 10px; color: ${p.gray}; text-transform: uppercase; letter-spacing: 0.3px; margin-top: 2px; }
  .section-title { font-size: 11px; font-weight: 500; color: ${p.gray}; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 10px; }
  .tab-row { display: flex; gap: 6px; margin-bottom: 16px; }
  .tab-btn { flex: 1; padding: 8px 4px; border-radius: 8px; font-family: 'DM Sans', sans-serif; font-size: 12px; font-weight: 500; cursor: pointer; transition: all 0.15s; }
  .tag { font-size: 11px; background: ${p.foam}; color: ${p.coffee}; border-radius: 5px; padding: 2px 7px; }
  .step-indicator { display: flex; justify-content: center; gap: 6px; margin-bottom: 16px; }
  .step-dot { width: 6px; height: 6px; border-radius: 50%; background: ${p.foam}; }
  .step-dot.active { background: ${p.caramel}; }
`;

export default function App() {
  const [employees, setEmployees] = useState(() => getStorage("ccc_employees", {}));
  const [records, setRecords] = useState(() => getStorage("ccc_records", []));
  const [currentUser, setCurrentUser] = useState(() => getStorage("ccc_user", null));
  const [screen, setScreen] = useState(currentUser ? "main" : "welcome");
  const [tab, setTab] = useState("check");
  const [regStep, setRegStep] = useState(1);
  const [regForm, setRegForm] = useState({ fullName: "", storeId: "", position: "", email: "" });
  const [newEmpId, setNewEmpId] = useState("");
  const [checkState, setCheckState] = useState({ storeId: "", employeeId: "", searching: false, result: null });
  const [adminTab, setAdminTab] = useState("hoy");
  const [sendingDigest, setSendingDigest] = useState(false);
  const [digestSent, setDigestSent] = useState(null);
  const [adminUnlocked, setAdminUnlocked] = useState(false);
  const [adminPin, setAdminPin] = useState("");

  function saveEmployees(emp) { setEmployees(emp); setStorage("ccc_employees", emp); }
  function saveRecords(recs) { setRecords(recs); setStorage("ccc_records", recs); }
  function saveUser(u) { setCurrentUser(u); setStorage("ccc_user", u); }

  function getStats(empId) {
    const now = new Date();
    const quinceStart = now.getDate() <= 15
      ? new Date(now.getFullYear(), now.getMonth(), 1)
      : new Date(now.getFullYear(), now.getMonth(), 16);
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
    const empRecs = records.filter(r => r.employeeId === empId);
    const lateMin = (list) => list.filter(r => r.lateMinutes > 0).reduce((s, r) => s + r.lateMinutes, 0);
    return {
      quinceMinutes: lateMin(empRecs.filter(r => new Date(r.timestamp) >= quinceStart)),
      monthMinutes: lateMin(empRecs.filter(r => new Date(r.timestamp) >= monthStart)),
      totalMinutes: lateMin(empRecs),
      totalChecks: empRecs.length,
      quinceAbsences: empRecs.filter(r => new Date(r.timestamp) >= quinceStart && r.type === "falta").length,
    };
  }

  function alertLevel(qMins) {
    if (qMins >= 120) return { cls: "info-red", msg: "⚠️ Más de 2 horas acumuladas esta quincena — riesgo de descuento de día." };
    if (qMins >= 60) return { cls: "info-amber", msg: "⏱ Más de 1 hora acumulada — procura llegar puntual los días restantes." };
    if (qMins >= 30) return { cls: "info-amber", msg: "⏱ 30+ minutos acumulados esta quincena." };
    return null;
  }

  async function handleRegister() {
    const { fullName, storeId, position, email } = regForm;
    const parts = fullName.trim().split(" ");
    const lastName = parts.length > 1 ? parts[parts.length - 1] : parts[0];
    const empId = generateEmployeeId(storeId, lastName, position);
    setNewEmpId(empId);
    const store = STORES.find(s => s.id === storeId);
    const user = { fullName, storeId, storeName: store.name, position, email, employeeId: empId, registeredAt: new Date().toISOString() };
    saveEmployees({ ...employees, [empId]: user });
    saveUser(user);
    setRegStep(3);
  }

  async function sendDigest(type) {
    setSendingDigest(true);
    setDigestSent(null);
    try {
      const res = await fetch("/api/send-digest", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ type, records, employees }),
      });
      const data = await res.json();
      setDigestSent(data.ok ? "enviado" : "error");
    } catch {
      setDigestSent("error");
    }
    setSendingDigest(false);
  }

  async function handleCheckIn() {
    if (!navigator.geolocation) {
      setCheckState(s => ({ ...s, result: { error: "Tu dispositivo no soporta geolocalización." } }));
      return;
    }
    setCheckState(s => ({ ...s, searching: true, result: null }));
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        const { latitude, longitude } = pos.coords;
        const store = STORES.find(s => s.id === checkState.storeId);
        const dist = haversine(latitude, longitude, store.lat, store.lng);
        if (dist > RADIUS_METERS) {
          setCheckState(s => ({ ...s, searching: false, result: { error: `Estás a ${Math.round(dist)}m de la tienda. Necesitas estar dentro de ${RADIUS_METERS}m para checar.` } }));
          return;
        }
        const now = new Date();
        const period = getCurrentPeriod();
        let lateMinutes = 0;
        let note = "";
        let shiftKey = "fuera_horario";
        if (period) {
          shiftKey = period.key;
          const nowMin = now.getHours() * 60 + now.getMinutes();
          const diff = nowMin - period.scheduledMin;
          if (diff >= LATE_MINUTES) lateMinutes = diff;
          if (period.key === "intermedio") {
            if (nowMin < 600 || nowMin > 660) note = "⚠️ Fuera de ventana 10-11am — Notificar a Admin (Victoria)";
          }
        }
        const emp = employees[checkState.employeeId];
        if (!emp) { setCheckState(s => ({ ...s, searching: false, result: { error: "Empleado no encontrado." } })); return; }
        const rec = {
          id: Date.now().toString(), employeeId: emp.employeeId, employeeName: emp.fullName,
          storeId: checkState.storeId, storeName: store.name, timestamp: now.toISOString(),
          lateMinutes, shift: shiftKey, note, type: "entrada", distance: Math.round(dist),
        };
        const newRecs = [rec, ...records];
        saveRecords(newRecs);
        // Notify if late
        if (lateMinutes > 0 || note) {
          fetch("/api/notify-late", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ record: rec, employee: emp }),
          }).catch(() => {});
        }
        setCheckState(s => ({ ...s, searching: false, result: { success: true, rec, emp } }));
      },
      () => setCheckState(s => ({ ...s, searching: false, result: { error: "No se pudo obtener tu ubicación. Verifica que diste permiso de ubicación." } })),
      { enableHighAccuracy: true, timeout: 15000 }
    );
  }

  const now = new Date();
  const period = getCurrentPeriod();
  const stats = currentUser ? getStats(currentUser.employeeId) : null;
  const al = stats ? alertLevel(stats.quinceMinutes) : null;
  const myRecs = records.filter(r => r.employeeId === currentUser?.employeeId).slice(0, 10);

  // ─── WELCOME / REGISTER ───────────────────────────────────────────────────
  if (screen === "welcome") {
    return (
      <>
        <style>{css}</style>
        <div className="app">
          <div className="header">
            <h1>☕ Che Che Café</h1>
            <p>Sistema de Asistencia</p>
          </div>
          <div className="content">
            {regStep === 1 && (
              <>
                <div style={{ textAlign: "center", padding: "24px 0 28px" }}>
                  <div style={{ fontSize: 60, marginBottom: 10 }}>☕</div>
                  <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 21, color: p.coffee, marginBottom: 8 }}>Bienvenido al checador</div>
                  <div style={{ fontSize: 14, color: p.gray, lineHeight: 1.6 }}>Regístrate una sola vez y listo.<br />Después solo checarás con tu nombre.</div>
                </div>
                <div className="card">
                  <div className="card-title">Crear cuenta</div>
                  <label>Nombre completo</label>
                  <input type="text" placeholder="Ej. Ana García López" value={regForm.fullName}
                    onChange={e => setRegForm(f => ({ ...f, fullName: e.target.value }))} />
                  <label>Tu tienda</label>
                  <select value={regForm.storeId} onChange={e => setRegForm(f => ({ ...f, storeId: e.target.value }))}>
                    <option value="">Selecciona tu tienda</option>
                    {STORES.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                  </select>
                  <label>Tu puesto</label>
                  <select value={regForm.position} onChange={e => setRegForm(f => ({ ...f, position: e.target.value }))}>
                    <option value="">Selecciona tu puesto</option>
                    {POSITIONS.map(p => <option key={p} value={p}>{p}</option>)}
                  </select>
                  <label>Correo electrónico</label>
                  <input type="email" placeholder="tu@email.com" value={regForm.email}
                    onChange={e => setRegForm(f => ({ ...f, email: e.target.value }))} />
                  <button className="btn btn-primary" onClick={() => setRegStep(2)}
                    disabled={!regForm.fullName || !regForm.storeId || !regForm.position || !regForm.email}>
                    Continuar →
                  </button>
                </div>
              </>
            )}
            {regStep === 2 && (
              <div className="card">
                <div className="card-title">Confirma tus datos</div>
                <div className="stat-row"><span className="stat-label">Nombre</span><span className="stat-value">{regForm.fullName}</span></div>
                <div className="stat-row"><span className="stat-label">Tienda</span><span className="stat-value">{STORES.find(s => s.id === regForm.storeId)?.name}</span></div>
                <div className="stat-row"><span className="stat-label">Puesto</span><span className="stat-value">{regForm.position}</span></div>
                <div className="stat-row" style={{ marginBottom: 16 }}><span className="stat-label">Correo</span><span className="stat-value" style={{ fontSize: 13 }}>{regForm.email}</span></div>
                <div className="info-box info-amber" style={{ marginTop: 4 }}>
                  <strong>Política de retardos:</strong> A partir de 15 minutos después de tu hora de entrada se registra retardo. Los minutos se acumulan cada quincena.
                </div>
                <button className="btn btn-primary" onClick={handleRegister}>✅ Registrarme</button>
                <button className="btn btn-secondary" onClick={() => setRegStep(1)}>← Regresar</button>
              </div>
            )}
            {regStep === 3 && (
              <div className="card" style={{ textAlign: "center" }}>
                <div style={{ fontSize: 50, margin: "10px 0 14px" }}>🎉</div>
                <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, color: p.coffee, marginBottom: 8 }}>¡Cuenta creada!</div>
                <div style={{ fontSize: 13, color: p.gray, marginBottom: 18 }}>Este es tu ID de empleado. Guárdalo.</div>
                <div className="id-badge">{newEmpId}</div>
                <div style={{ fontSize: 12, color: p.gray, margin: "12px 0 20px" }}>
                  Lo necesitarás para checar si cambias de dispositivo.
                </div>
                <button className="btn btn-primary" onClick={() => { setScreen("main"); setTab("check"); }}>
                  Comenzar →
                </button>
              </div>
            )}
          </div>
        </div>
      </>
    );
  }

  // ─── MAIN APP ─────────────────────────────────────────────────────────────
  return (
    <>
      <style>{css}</style>
      <div className="app">
        <div className="header">
          <h1>☕ Che Che Café</h1>
          {currentUser && <p>{currentUser.fullName} · {currentUser.storeName}</p>}
        </div>

        <div className="content">

          {/* ── CHECK ── */}
          {tab === "check" && (
            <>
              {period && (
                <div className="info-box info-green">
                  <strong>Turno activo:</strong> {SCHEDULES[period.key].label} —{" "}
                  {period.key === "intermedio" ? "10:00 – 11:00 AM" : (() => { const t = SCHEDULES[period.key].getTime(now.getDay()); return `${t.h}:${String(t.m).padStart(2,"0")}`; })()}
                </div>
              )}
              {!period && (
                <div className="info-box info-amber">No hay turno activo en este momento.</div>
              )}

              {checkState.result?.success ? (
                <div className="card" style={{ textAlign: "center" }}>
                  <div style={{ fontSize: 52, margin: "6px 0 10px" }}>{checkState.result.rec.lateMinutes > 0 ? "⏰" : "✅"}</div>
                  <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, color: p.coffee, marginBottom: 6 }}>
                    {checkState.result.rec.lateMinutes > 0 ? `Retardo — ${formatMin(checkState.result.rec.lateMinutes)}` : "¡Entrada registrada!"}
                  </div>
                  <div style={{ fontSize: 13, color: p.gray, marginBottom: 16 }}>
                    {new Date(checkState.result.rec.timestamp).toLocaleTimeString("es-MX", { hour: "2-digit", minute: "2-digit" })} · {checkState.result.rec.storeName}
                    {" · "}{Math.round(checkState.result.rec.distance)}m de distancia
                  </div>
                  {checkState.result.rec.lateMinutes > 0 && (
                    <div className="info-box info-amber" style={{ textAlign: "left" }}>
                      Retardo de <strong>{formatMin(checkState.result.rec.lateMinutes)}</strong>. Acumulado quincena: <strong>{formatMin(stats?.quinceMinutes || 0)}</strong>
                    </div>
                  )}
                  {checkState.result.rec.note && (
                    <div className="info-box info-red" style={{ textAlign: "left" }}>{checkState.result.rec.note}</div>
                  )}
                  <button className="btn btn-secondary" onClick={() => setCheckState({ storeId: currentUser?.storeId || "", employeeId: currentUser?.employeeId || "", searching: false, result: null })}>
                    Nueva entrada
                  </button>
                </div>
              ) : (
                <div className="card">
                  <div className="card-title">Registrar entrada</div>
                  <label>Tienda</label>
                  <select value={checkState.storeId} onChange={e => setCheckState(s => ({ ...s, storeId: e.target.value, employeeId: "" }))}>
                    <option value="">Selecciona tienda</option>
                    {STORES.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                  </select>
                  <label>Empleado</label>
                  <select value={checkState.employeeId} onChange={e => setCheckState(s => ({ ...s, employeeId: e.target.value }))}>
                    <option value="">Selecciona empleado</option>
                    {Object.values(employees)
                      .filter(e => !checkState.storeId || e.storeId === checkState.storeId)
                      .map(e => <option key={e.employeeId} value={e.employeeId}>{e.fullName}</option>)}
                  </select>
                  {checkState.result?.error && (
                    <div className="info-box info-red">{checkState.result.error}</div>
                  )}
                  <button className="btn btn-check"
                    disabled={!checkState.storeId || !checkState.employeeId || checkState.searching}
                    onClick={handleCheckIn}>
                    {checkState.searching ? "Verificando ubicación..." : "📍 Checar entrada"}
                  </button>
                  <div style={{ fontSize: 12, color: p.gray, textAlign: "center", marginTop: 10 }}>
                    Debes estar dentro de {RADIUS_METERS}m de la tienda
                  </div>
                </div>
              )}

              {myRecs.length > 0 && (
                <div className="card">
                  <div className="card-title" style={{ marginBottom: 10 }}>Mis últimas entradas</div>
                  {myRecs.slice(0, 5).map(r => (
                    <div key={r.id} className="rec-row">
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div>
                          <div className="rec-name">{r.storeName}</div>
                          <div className="rec-meta">
                            {new Date(r.timestamp).toLocaleDateString("es-MX", { weekday: "short", day: "numeric", month: "short" })} · {new Date(r.timestamp).toLocaleTimeString("es-MX", { hour: "2-digit", minute: "2-digit" })} · {SCHEDULES[r.shift]?.label || r.shift}
                          </div>
                        </div>
                        {r.lateMinutes > 0 ? <span className="badge badge-red">+{formatMin(r.lateMinutes)}</span> : <span className="badge badge-green">Puntual</span>}
                      </div>
                    </div>
                  ))}
                </div>
              )}
            </>
          )}

          {/* ── PROFILE ── */}
          {tab === "profile" && currentUser && (
            <>
              <div className="card">
                <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 16 }}>
                  <div style={{ width: 54, height: 54, borderRadius: "50%", background: p.foam, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, color: p.coffee, fontWeight: 700, flexShrink: 0 }}>
                    {currentUser.fullName.split(" ").map(n => n[0]).slice(0, 2).join("")}
                  </div>
                  <div>
                    <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 17, color: p.coffee }}>{currentUser.fullName}</div>
                    <div style={{ fontSize: 13, color: p.gray }}>{currentUser.storeName}</div>
                    <div style={{ fontSize: 12, color: p.gray }}>{currentUser.position} · <span style={{ fontFamily: "monospace", fontWeight: 700 }}>{currentUser.employeeId}</span></div>
                  </div>
                </div>
              </div>

              {al && <div className={`info-box ${al.cls}`}>{al.msg}</div>}

              <div className="card">
                <div className="card-title">Mis acumulados</div>
                <div className="stat-row">
                  <span className="stat-label">Retardos esta quincena</span>
                  <span className="stat-value" style={{ color: stats.quinceMinutes > 0 ? p.amber : p.green }}>{formatMin(stats.quinceMinutes)}</span>
                </div>
                <div className="stat-row">
                  <span className="stat-label">Retardos este mes</span>
                  <span className="stat-value">{formatMin(stats.monthMinutes)}</span>
                </div>
                <div className="stat-row">
                  <span className="stat-label">Histórico total</span>
                  <span className="stat-value">{formatMin(stats.totalMinutes)}</span>
                </div>
                <div className="stat-row">
                  <span className="stat-label">Faltas quincena</span>
                  <span className="stat-value" style={{ color: stats.quinceAbsences > 0 ? p.red : p.green }}>{stats.quinceAbsences}</span>
                </div>
                <div className="stat-row">
                  <span className="stat-label">Entradas registradas</span>
                  <span className="stat-value">{stats.totalChecks}</span>
                </div>
              </div>

              <div className="card">
                <div className="card-title">Política de retardos</div>
                {[
                  { dot: p.green, label: "0–14 min después de entrada", badge: "badge-green", text: "Puntual" },
                  { dot: p.amber, label: "15–59 min tarde", badge: "badge-amber", text: "Retardo" },
                  { dot: p.red, label: "60+ min acumulados/quincena", badge: "badge-red", text: "Descuento posible" },
                  { dot: p.red, label: "120+ min acumulados/quincena", badge: "badge-red", text: "Descuento de día" },
                ].map((row, i) => (
                  <div key={i} className="stat-row" style={{ alignItems: "center" }}>
                    <div style={{ display: "flex", alignItems: "center" }}>
                      <span className="dot" style={{ background: row.dot }} />
                      <span className="stat-label">{row.label}</span>
                    </div>
                    <span className={`badge ${row.badge}`}>{row.text}</span>
                  </div>
                ))}
              </div>

              <button className="btn btn-secondary" onClick={() => {
                localStorage.removeItem("ccc_user");
                setCurrentUser(null);
                setScreen("welcome");
                setRegStep(1);
                setRegForm({ fullName: "", storeId: "", position: "", email: "" });
              }}>
                Cerrar sesión / Cambiar cuenta
              </button>
            </>
          )}

          {/* ── ADMIN ── */}
          {tab === "admin" && !adminUnlocked && (
            <div className="card" style={{textAlign:"center"}}>
              <div style={{fontSize:44,margin:"10px 0 14px"}}>🔒</div>
              <div className="card-title">Área Admin</div>
              <div style={{fontSize:13,color:p.gray,marginBottom:16}}>Ingresa el PIN para continuar</div>
              <input type="password" placeholder="• • • •" value={adminPin}
                onChange={e => setAdminPin(e.target.value)}
                onKeyDown={e => { if(e.key==="Enter"){ if(adminPin==="5366") setAdminUnlocked(true); else { alert("PIN incorrecto"); setAdminPin(""); }}}}
                style={{textAlign:"center",letterSpacing:8,fontSize:20,marginBottom:14}} />
              <button className="btn btn-primary" onClick={() => {
                if(adminPin==="5366") setAdminUnlocked(true);
                else { alert("PIN incorrecto"); setAdminPin(""); }
              }}>Entrar</button>
            </div>
          )}
          {tab === "admin" && adminUnlocked && (
            <>
              <div className="tab-row">
                {["hoy", "empleados", "digest"].map(t => (
                  <button key={t} className="tab-btn"
                    onClick={() => setAdminTab(t)}
                    style={{
                      background: adminTab === t ? p.coffee : "white",
                      color: adminTab === t ? p.cream : p.coffee,
                      border: `1.5px solid ${adminTab === t ? p.coffee : p.foam}`,
                    }}>
                    {t === "hoy" ? "Hoy" : t === "empleados" ? "Empleados" : "Digest"}
                  </button>
                ))}
              </div>

              {adminTab === "hoy" && (
                <div className="card">
                  <div className="card-title">Registros de hoy — {now.toLocaleDateString("es-MX", { weekday: "long", day: "numeric", month: "long" })}</div>
                  {STORES.map(store => {
                    const sr = records.filter(r => r.storeId === store.id && new Date(r.timestamp).toDateString() === now.toDateString());
                    if (!sr.length) return null;
                    return (
                      <div key={store.id} style={{ marginBottom: 16 }}>
                        <div className="section-title">{store.name}</div>
                        {sr.map(r => (
                          <div key={r.id} className="rec-row">
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                              <div>
                                <div className="rec-name">{r.employeeName}</div>
                                <div className="rec-meta">{new Date(r.timestamp).toLocaleTimeString("es-MX", { hour: "2-digit", minute: "2-digit" })} · {SCHEDULES[r.shift]?.label || r.shift}</div>
                                {r.note && <div style={{ fontSize: 11, color: p.red, marginTop: 2 }}>{r.note}</div>}
                              </div>
                              {r.lateMinutes > 0 ? <span className="badge badge-red">+{formatMin(r.lateMinutes)}</span> : <span className="badge badge-green">Puntual</span>}
                            </div>
                          </div>
                        ))}
                      </div>
                    );
                  })}
                  {!records.some(r => new Date(r.timestamp).toDateString() === now.toDateString()) && (
                    <div style={{ textAlign: "center", color: p.gray, fontSize: 13, padding: "20px 0" }}>Sin registros hoy</div>
                  )}
                </div>
              )}

              {adminTab === "empleados" && (
                <div className="card">
                  <div className="card-title">Empleados ({Object.keys(employees).length})</div>
                  {STORES.map(store => {
                    const emps = Object.values(employees).filter(e => e.storeId === store.id);
                    if (!emps.length) return null;
                    return (
                      <div key={store.id} style={{ marginBottom: 16 }}>
                        <div className="section-title">{store.name}</div>
                        {emps.map(e => {
                          const s = getStats(e.employeeId);
                          return (
                            <div key={e.employeeId} className="rec-row">
                              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                <div>
                                  <div className="rec-name">{e.fullName}</div>
                                  <div className="rec-meta">{e.employeeId} · {e.position}</div>
                                </div>
                                <span className={`badge ${s.quinceMinutes >= 60 ? "badge-red" : s.quinceMinutes > 0 ? "badge-amber" : "badge-green"}`}>
                                  {formatMin(s.quinceMinutes)} qna
                                </span>
                              </div>
                            </div>
                          );
                        })}
                      </div>
                    );
                  })}
                  {!Object.keys(employees).length && (
                    <div style={{ textAlign: "center", color: p.gray, fontSize: 13, padding: "20px 0" }}>Sin empleados registrados</div>
                  )}
                </div>
              )}

              {adminTab === "digest" && (
                <div className="card">
                  <div className="card-title">Digest de asistencia</div>
                  <div style={{ fontSize: 13, color: p.gray, marginBottom: 16, lineHeight: 1.7 }}>
                    Envía el resumen de asistencia clasificado por tienda a:<br />
                    <span style={{ fontFamily: "monospace", fontSize: 12 }}>luiscgarinian@me.com · ferhughes04@gmail.com · saiv1221hughes@gmail.com</span>
                  </div>
                  {["diario", "semanal", "quincenal"].map(type => (
                    <button key={type} className="btn btn-primary" style={{ marginBottom: 10 }}
                      onClick={() => sendDigest(type)} disabled={sendingDigest}>
                      {sendingDigest ? "Enviando..." : `📧 Enviar digest ${type}`}
                    </button>
                  ))}
                  {digestSent === "enviado" && <div className="info-box info-green">✅ Digest enviado correctamente.</div>}
                  {digestSent === "error" && <div className="info-box info-red">Error al enviar. Verifica la configuración de Resend.</div>}

                  <div style={{ marginTop: 8 }}>
                    <div className="section-title" style={{ marginBottom: 12 }}>Resumen actual por tienda</div>
                    {STORES.map(store => {
                      const emps = Object.values(employees).filter(e => e.storeId === store.id);
                      if (!emps.length) return null;
                      const todayRecs = records.filter(r => r.storeId === store.id && new Date(r.timestamp).toDateString() === now.toDateString());
                      return (
                        <div key={store.id} style={{ marginBottom: 14 }}>
                          <div className="section-title">{store.name}</div>
                          <div className="mini-grid">
                            <div className="mini-card"><div className="mini-val">{emps.length}</div><div className="mini-lbl">Empleados</div></div>
                            <div className="mini-card"><div className="mini-val">{todayRecs.filter(r => r.lateMinutes === 0).length}</div><div className="mini-lbl">Puntual</div></div>
                            <div className="mini-card"><div className="mini-val">{todayRecs.filter(r => r.lateMinutes > 0).length}</div><div className="mini-lbl">Retardos</div></div>
                          </div>
                        </div>
                      );
                    })}
                  </div>
                </div>
              )}
              <button className="btn btn-secondary" style={{marginTop:4}} onClick={() => { setAdminUnlocked(false); setAdminPin(""); }}>
                🔒 Cerrar Admin
              </button>
            </>
          )}
        </div>

        <nav className="nav">
          {[
            { key: "check", icon: "📍", label: "Checar" },
            { key: "profile", icon: "👤", label: "Mi perfil" },
            { key: "admin", icon: "📊", label: "Admin" },
          ].map(t => (
            <button key={t.key} className={`nav-btn ${tab === t.key ? "active" : ""}`} onClick={() => setTab(t.key)}>
              <span className="nav-icon">{t.icon}</span>{t.label}
            </button>
          ))}
        </nav>
      </div>
    </>
  );
}
