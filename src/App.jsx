...
const APPROVED_EXPENSE_KEYWORDS = ["café", "cafe", "leche", "hielo", "azúcar", "azucar", "servilleta", "vaso", "tapa", "popote", "limpieza", "detergente", "papel", "bolsa", "agua"];
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .info-blue { background: #E3F2FD; border-color: #1565C0; color: #1565C0; }
  ...
  .mini-val { font-size: 18px; font-weight: 500; color: ${p.coffee}; }
  ...
  .photo-btn:hover { border-color: ${p.caramel}; color: ${p.caramel}; }
  .shift-card { border: 2px solid ${p.foam}; border-radius: 12px; padding: 14px; margin-bottom: 10px; }
  .shift-card.active { border-color: ${p.caramel}; background: #FFFBF5; }
  .divider { height: 1px; background: ${p.foam}; margin: 16px 0; }
`;
...
export default function App() {
  ...
  const [checkoutForm, setCheckoutForm] = useState({ totalCorte: "", propinas: "", gastos: [{ concepto: "", monto: "", foto: null, fotoPreview: null }], notas: "", ticketFotos: [] });
  const [checkoutStep, setCheckoutStep] = useState("select"); // select | form | done
  ...
  useEffect(() => { if (currentUser) { loadMyRecords(); loadStats(); } }, [currentUser]);
  ...
  async function loadAdminData() {
    ...
    const { data: cutsData } = await supabase.from("cuts").select("*").order("timestamp", { ascending: false }).limit(100);
    if (emps) setEmployees(emps);
    if (recs) setRecords(recs);
    if (cutsData) setCuts(cutsData);
  ...
  async function handleCheckIn() {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        if (dist > RADIUS_METERS) { setCheckState(s => ({ ...s, searching: false, result: { error: `Estás a ${Math.round(dist)}m de la tienda. Necesitas estar dentro de ${RADIUS_METERS}m.` } })); return; }
        const now = new Date();
      ...
      () => setCheckState(s => ({ ...s, searching: false, result: { error: "No se pudo obtener tu ubicación. Verifica que diste permiso." } })),
  ...
  async function handleCheckOut() {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        // Find last check-in for this employee today
  ...
  async function handleSubmitCut() {
    ...
    if (!empData) return;
    const now = new Date();
    const totalGastos = checkoutForm.gastos.reduce((s, g) => s + (parseFloat(g.monto) || 0), 0);
    const unapprovedExpenses = checkoutForm.gastos.filter(g => g.concepto && !isApprovedExpense(g.concepto));
    ...
    const cut = {
      ...
      total_corte: parseFloat(checkoutForm.totalCorte) || 0,
      propinas: parseFloat(checkoutForm.propinas) || 0,
      gastos: JSON.stringify(checkoutForm.gastos.filter(g => g.concepto || g.monto)),
    ...
    // Notify if unapproved expenses
    if (unapprovedExpenses.length > 0) {
      fetch("/api/notify-expense", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ cut, employee: empData, unapprovedExpenses }),
      }).catch(() => {});
    }

    setCheckoutStep("done");
    setCheckoutForm({ totalCorte: "", propinas: "", gastos: [{ concepto: "", monto: "", foto: null, fotoPreview: null }], notas: "", ticketFotos: [] });
  }

  function addGastoRow() {
    setCheckoutForm(f => ({ ...f, gastos: [...f.gastos, { concepto: "", monto: "", foto: null, fotoPreview: null }] }));
  }

  function removeGastoRow(i) {
    setCheckoutForm(f => ({ ...f, gastos: f.gastos.filter((_, idx) => idx !== i) }));
  }

  function updateGasto(i, field, val) {
    setCheckoutForm(f => ({ ...f, gastos: f.gastos.map((g, idx) => idx === i ? { ...g, [field]: val } : g) }));
  }
  ...
  function handleTicketPhoto(file) {
    if (!file) return;
    const reader = new FileReader();
    reader.onload = (e) => setCheckoutForm(f => ({ ...f, ticketFotos: [...f.ticketFotos, e.target.result] }));
    reader.readAsDataURL(file);
  }
  ...
  // ─── CHECKOUT FLOW ────────────────────────────────────────────────────────
  if (tab === "check" && checkoutStep === "form") {
    const totalGastos = checkoutForm.gastos.reduce((s, g) => s + (parseFloat(g.monto) || 0), 0);
    return (
      <>
        <style>{css}</style>
        <div className="app">
          ...
          <div className="content">
            <div className="info-box info-blue">Registra el corte de caja de tu turno antes de salir.</div>

            <div className="card">
              <div className="card-title">💰 Ventas del turno</div>
              <label>Total del corte (MXN)</label>
              <input type="number" placeholder="0.00" value={checkoutForm.totalCorte} onChange={e => setCheckoutForm(f => ({ ...f, totalCorte: e.target.value }))} />
              <label>Propinas del turno (MXN)</label>
              <input type="number" placeholder="0.00" value={checkoutForm.propinas} onChange={e => setCheckoutForm(f => ({ ...f, propinas: e.target.value }))} />
            </div>

            <div className="card">
              <div className="card-title">🧾 Gastos del turno</div>
              <div className="info-box info-amber">
                <strong>Solo gastos de primera necesidad:</strong> café, leche, hielo, artículos necesarios para operación. Cualquier otro gasto debe ser autorizado por Victoria previamente.
              </div>
                ...
                return (
                  <div key={i}>
                    <div className="expense-row">
                      <input type="text" placeholder="Concepto (ej. leche, hielo)" value={g.concepto}
                      ...
                      {checkoutForm.gastos.length > 1 && (
                        <button className="remove-btn" onClick={() => removeGastoRow(i)}>✕</button>
                      )}
                    </div>
                    {g.concepto && !approved && (
                      <div className="info-box info-red" style={{ marginTop: 4, marginBottom: 8 }}>
                        ⚠️ Este gasto requiere autorización de Victoria. Se enviará notificación automática.
                      </div>
                    )}
                    {g.concepto && approved && (
                      <div className="info-box info-green" style={{ marginTop: 4, marginBottom: 8 }}>
                        ✓ Gasto aprobado
                      </div>
                    )}
                    <div style={{ marginBottom: 10 }}>
                      <input type="file" accept="image/*" capture="environment" style={{ display: "none" }} id={`foto-${i}`}
                        onChange={e => handleGastoPhoto(i, e.target.files[0])} />
                      {g.fotoPreview
                        ? <img src={g.fotoPreview} className="photo-preview" alt="ticket" />
                        : <label htmlFor={`foto-${i}`} className="photo-btn">📷 Foto del ticket de este gasto</label>
                      }
                    </div>
                  </div>
                );
              })}
              <button className="btn btn-secondary" onClick={addGastoRow} style={{ marginTop: 0 }}>+ Agregar gasto</button>
              {totalGastos > 0 && (
                <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 0 0", borderTop: `1px solid ${p.foam}`, marginTop: 10 }}>
                  <span className="stat-label">Total gastos</span>
                  <span style={{ fontWeight: 500, color: p.coffee }}>{formatCurrency(totalGastos)}</span>
                </div>
              )}
            </div>

            <div className="card">
              <div className="card-title">📸 Tickets generales</div>
              <div style={{ fontSize: 13, color: p.gray, marginBottom: 12 }}>Fotografía todos los tickets de compras del turno.</div>
              {checkoutForm.ticketFotos.map((foto, i) => (
                <img key={i} src={foto} className="photo-preview" alt={`ticket-${i}`} />
              ))}
              <input type="file" accept="image/*" capture="environment" style={{ display: "none" }} ref={ticketInputRef}
                onChange={e => handleTicketPhoto(e.target.files[0])} />
              <button className="photo-btn" onClick={() => ticketInputRef.current?.click()}>
                📷 Agregar foto de ticket ({checkoutForm.ticketFotos.length} fotos)
              </button>
            </div>
            ...
            <button className="btn btn-primary" onClick={handleSubmitCut} disabled={!checkoutForm.totalCorte}>
              ✅ Registrar corte y salida
            </button>
            <button className="btn btn-secondary" onClick={() => setCheckoutStep("select")}>← Cancelar</button>
          </div>
        </div>
      </>
    );
        ...
        <div className="app">
          ...
          <div className="content">
            <div className="card" style={{ textAlign: "center" }}>
              ...
              <div style={{ fontSize: 13, color: p.gray, marginBottom: 20 }}>Tu salida y corte de caja quedaron registrados.</div>
              <div style={{ background: p.milk, borderRadius: 10, padding: 14, marginBottom: 16, textAlign: "left" }}>
                ...
                <div className="stat-row"><span className="stat-label">Propinas</span><span className="stat-value">{formatCurrency(checkoutForm.propinas)}</span></div>
              </div>
              <button className="btn btn-primary" onClick={() => { setCheckoutStep("select"); setCheckState(s => ({ ...s, result: null, mode: "entrada" })); }}>
                Listo
              </button>
            </div>
          </div>
            ...
            {checkState.result?.success ? (
              ...
              <div className="card">
                ...
                <button className="btn btn-checkout"
                  disabled={!checkState.storeId || !checkState.employeeId || checkState.searching}
                  onClick={() => { setCheckState(s => ({ ...s, mode: "salida" })); handleCheckOut(); }}>
                  {checkState.searching && checkState.mode === "salida" ? "Verificando..." : "🚪 Check out — Salida + Corte"}
                </button>
                  ...
                  return (
                    <div key={store.id} style={{ marginBottom: 20 }}>
                      ...
                      <div className="mini-grid">
                        <div className="mini-card"><div className="mini-val" style={{ fontSize: 14 }}>{formatCurrency(totalVentas)}</div><div className="mini-lbl">Ventas</div></div>
                        <div className="mini-card"><div className="mini-val" style={{ fontSize: 14 }}>{formatCurrency(totalPropinas)}</div><div className="mini-lbl">Propinas</div></div>
                        <div className="mini-card"><div className="mini-val" style={{ fontSize: 14 }}>{formatCurrency(totalGastos)}</div><div className="mini-lbl">Gastos</div></div>
                      </div>
                      {storeCuts.map(c => (
                        <div key={c.id} className="rec-row">
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                            <div>
                              ...
                              <div className="rec-meta">Corte: {formatCurrency(c.total_corte)} · Propinas: {formatCurrency(c.propinas)} · Gastos: {formatCurrency(c.total_gastos)}</div>
