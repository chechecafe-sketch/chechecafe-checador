...
export default function App() {
  ...
  useEffect(()=>{
    if(currentUser) { setScreen("main"); loadMyRecords(); loadStats(); loadAllCuts(); }
    else setScreen("auth");
  },[currentUser]);
  ...
  useEffect(()=>{
    ...
    const interval = setInterval(()=>{ setAutoRefresh(n=>n+1); },30000);
    return ()=>clearInterval(interval);
  },[adminUnlocked]);

  useEffect(()=>{ if(adminUnlocked) loadAdminData(); },[adminUnlocked,autoRefresh]);
  ...
  async function handleCheckIn() {
    ...
    navigator.geolocation.getCurrentPosition(
      async(pos)=>{
        ...
        const isDemoMode=currentUser.username?.includes("elmaschingon");
  ...
  if(tab==="check"&&checkoutStep==="form") return(
    ...
    <div className="app">
      ...
      <div className="content">
        ...
        <button className="btn btn-primary" onClick={handleSubmitCut} disabled={!checkoutForm.totalCorte||isOverspent}>✅ Registrar corte</button>
      ...
      <div className="content">
        ...
        {tab==="admin"&&adminUnlocked&&(<>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div style={{fontSize:11,color:p.gray}}>↻ Auto-actualiza cada 30s</div>
