...
const KAIZEEN_PRODUCTS = [
  { id: "ROL_CANELA",           name: "Rol de Canela",                  category: "Tradicionales",  price: 32 },
  { id: "CROISSANT_NATURAL",    name: "Croissant Natural",              category: "Croissant",      price: 32 },
  { id: "CROISSANT_ALMENDRA",   name: "Croissant Almendra",             category: "Croissant",      price: 38 },
  { id: "CROISSANT_CHOCOLATE",  name: "Croissant Chocolate",            category: "Croissant",      price: 38 },
  { id: "CROISSANT_FERRERO",    name: "Croissant Ferrero",              category: "Croissant",      price: 40 },
  { id: "CROISSANT_MATCHA",     name: "Croissant Matcha",               category: "Croissant",      price: 40 },
  { id: "CROISSANT_PISTACHE",   name: "Croissant Roll Pistache",        category: "Croissant Roll", price: 42 },
  { id: "NIDO_GUAYABA",         name: "Nido de Guayaba",                category: "Nidos",          price: 47 },
  { id: "GALLETA_SAL",          name: "Galleta Choco y Flor de Sal",    category: "Galletas",       price: 32 },
  { id: "GALLETA_ARANDANO",     name: "Galleta Arándano Choco Blanco",  category: "Galletas",       price: 32 },
  { id: "GALLETA_TRIPLE_CHOCO", name: "Galleta Triple Chocolate",       category: "Galletas",       price: 32 },
  { id: "PANQUE_LIMON",         name: "Panqué de Limón",                category: "Especiales",     price: 38 },
  { id: "CONCHA_VAINILLA",      name: "Concha Vainilla",                category: "Especiales",     price: 38 },
  { id: "CONCHA_CHOCOLATE",     name: "Concha Chocolate",               category: "Especiales",     price: 38 },
];

const WEEKEND_BOOST = 1.3;
const ORDER_DEADLINE_HOUR = 18;
const ENVIO_KAIZEEN = 50;
...
function getCurrentPeriod() {
  ...
  for(const [key,sched] of Object.entries(SCHEDULES)) {
    const {h:sh,m:sm}=sched.getTime(day),schedMin=sh*60+sm;
...
const css=`
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  .nav-btn{flex:1;padding:12px 4px;border:none;background:transparent;color:${p.foam};font-size:10px;font-family:'DM Sans',sans-serif;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:2px;transition:background 0.15s;}
  ...
  .toggle-track{width:44px;height:24px;border-radius:12px;background:${p.foam};cursor:pointer;position:relative;transition:background 0.2s;flex-shrink:0;}
  .toggle-thumb{position:absolute;width:18px;height:18px;border-radius:50%;background:white;top:3px;transition:left 0.2s;}
`;
...
export default function App() {
  ...
  // ── PEDIDOS STATE ──────────────────────────────────────────────────────────
  const [orderTab, setOrderTab]               = useState("levantar");
  ...
  const [isWeekendBoost, setIsWeekendBoost]   = useState(() => { const d=new Date().getDay(); return d===5||d===6||d===0; });
  const [eventBoost, setEventBoost]           = useState(false);
  const [eventName, setEventName]             = useState("");

  // ── EFFECTS ────────────────────────────────────────────────────────────────
  useEffect(()=>{
    if(currentUser){ setScreen("main"); loadMyRecords(); loadStats(); loadAllCuts(); loadMaximos(); loadMyOrders(); if(SUPER_ADMIN_USERS.includes(currentUser.username)){setAdminUnlocked(true);} }
    else setScreen("auth");
  },[currentUser]);
  ...
  useEffect(()=>{
    if(tab==="pedidos"){ loadMaximos(); loadMyOrders(); }
    if(tab==="admin"&&adminUnlocked){ loadAllOrders(); }
  },[tab]);
  ...
  async function loadAdminData() {
    ...
    const {data:cutsData}=await supabase.from("cuts").select("id,employee_id,employee_name,store_id,store_name,timestamp,total_corte,propinas,total_gastos,notas,tiene_gastos_no_aprobados,es_cajero").order("timestamp",{ascending:false}).limit(200);
  ...
  // ── PEDIDOS HELPERS ────────────────────────────────────────────────────────
  function getBoost() { return (isWeekendBoost||eventBoost) ? WEEKEND_BOOST : 1.0; }

  function getMax(productId) {
    const m=maximos.find(x=>x.product_id===productId);
    if(!m) return 0;
    return Math.ceil(m.max_qty*getBoost());
  }

  function getOrder(productId) {
    const stock=parseInt(stockInputs[productId]);
    if(isNaN(stock)||stock<0) return null;
    return Math.max(0, getMax(productId)-stock);
  }

  function getTotalPiezas() {
    return KAIZEEN_PRODUCTS.reduce((acc,prod)=>{ const o=getOrder(prod.id); return acc+(o!==null?o:0); },0);
  }

  function getTotalCost() {
    return KAIZEEN_PRODUCTS.reduce((acc,prod)=>{ const o=getOrder(prod.id); return acc+(o!==null?o*prod.price:0); },0)+ENVIO_KAIZEEN;
  }

  function isOverstocked(productId) {
    const stock=parseInt(stockInputs[productId]);
    if(isNaN(stock)) return false;
    return stock>=getMax(productId)&&getMax(productId)>0;
  }

  function hasAnomalies() { return KAIZEEN_PRODUCTS.some(prod=>isOverstocked(prod.id)); }

  // ── PEDIDOS DATA ───────────────────────────────────────────────────────────
  async function loadMaximos() {
    if(!currentUser) return;
    const {data}=await supabase.from("store_maximos").select("product_id,max_qty").eq("store_id",currentUser.storeId);
    if(data) setMaximos(data);
  }

  async function loadMyOrders() {
    if(!currentUser) return;
    const {data}=await supabase.from("orders").select("*, order_items(*)").eq("store_id",currentUser.storeId).order("submitted_at",{ascending:false}).limit(10);
    if(data) setMyOrders(data);
  }

  async function loadAllOrders() {
    setLoadingOrders(true);
    const {data}=await supabase.from("orders").select("*, order_items(*)").order("submitted_at",{ascending:false}).limit(50);
  ...
  async function handleSubmitOrder() {
    ...
    const items=KAIZEEN_PRODUCTS
      .map(prod=>({ product_id:prod.id, stock_reported:parseInt(stockInputs[prod.id])||0, qty_ordered:getOrder(prod.id)||0, unit_price_snapshot:prod.price }))
      .filter(i=>!isNaN(parseInt(stockInputs[i.product_id])));
    if(!items.length){ alert("Captura el stock de al menos un producto."); setSubmittingOrder(false); return; }
    const store=STORES.find(s=>s.id===currentUser.storeId);
    const {data:order,error}=await supabase.from("orders").insert({
      store_id:currentUser.storeId, store_name:store?.name||currentUser.storeName,
      submitted_by:currentUser.fullName, status:"pending",
      total_cost:getTotalCost(), submitted_at:new Date().toISOString(),
    }).select().single();
    if(error||!order){ alert("Error al guardar el pedido. Intenta de nuevo."); setSubmittingOrder(false); return; }
    await supabase.from("order_items").insert(items.map(i=>({...i,order_id:order.id})));
    setOrderResult({order,items,totalCost:getTotalCost()});
    setStockInputs({}); setOrderNote("");
    await loadMyOrders();
    setSubmittingOrder(false);
  }

  async function handleApproveOrder(orderId) {
    const note=reviewNote[orderId]||"";
    const order=allOrders.find(o=>o.id===orderId);
    const hasFlags=order?.order_items?.some(i=>{ const m=maximos.find(x=>x.product_id===i.product_id); return m&&i.stock_reported>=m.max_qty; });
    if(hasFlags&&!note.trim()){ alert("Este pedido tiene alertas de stock. Escribe una nota antes de aprobar."); return; }
    await supabase.from("orders").update({ status:"approved", reviewer_name:currentUser.fullName, reviewer_note:note, approved_at:new Date().toISOString() }).eq("id",orderId);
    await loadAllOrders();
  }

  async function handleRejectOrder(orderId) {
    const note=reviewNote[orderId]||"";
    if(!note.trim()){ alert("Escribe el motivo del rechazo."); return; }
    await supabase.from("orders").update({ status:"rejected", reviewer_name:currentUser.fullName, reviewer_note:note }).eq("id",orderId);
    await loadAllOrders();
  }

  function buildKaizenMessage(ordersToSend) {
    const d=new Date();
    let msg=`🥐 PEDIDO CHE CHE CAFÉ\n`;
    msg+=`📅 ${d.toLocaleDateString("es-MX",{weekday:"long",day:"numeric",month:"long"})}\n`;
    msg+=`✅ Aprobado por: ${currentUser.fullName}\n\n`;
    ordersToSend.forEach(o=>{
      const piezas=o.order_items.filter(i=>i.qty_ordered>0);
      if(!piezas.length) return;
      msg+=`📍 ${o.store_name}\n`;
      piezas.forEach(i=>{ const prod=KAIZEEN_PRODUCTS.find(x=>x.id===i.product_id); msg+=`  • ${prod?.name||i.product_id}: ${i.qty_ordered} pz\n`; });
      msg+=`\n`;
    });
    const total=ordersToSend.reduce((a,o)=>a+(o.total_cost||0),0);
    msg+=`💰 Total estimado: $${Math.round(total).toLocaleString("es-MX")} MXN\n`;
    if(isWeekendBoost||eventBoost) msg+=`⭐ Máximos con +30% por temporada alta\n`;
    return msg;
  }

  function sendToKaizen() {
    const approved=allOrders.filter(o=>o.status==="approved");
    if(!approved.length){ alert("No hay pedidos aprobados para enviar."); return; }
    const msg=buildKaizenMessage(approved);
    window.open("https://wa.me/?text="+encodeURIComponent(msg),"_blank");
    approved.forEach(o=>{ supabase.from("orders").update({status:"sent"}).eq("id",o.id); });
  ...
  async function handleLogin() {
    ...
    if(!emp){setAuthError("Usuario no encontrado.");setAuthLoading(false);return;}
    if(!emp.password_hash){setSetupForm(f=>({...f,username}));setAuthMode("setup");setAuthLoading(false);return;}
    ...
    setLS("ccc_user_v2",user); setCurrentUser(user); setAuthLoading(false);
  }
  ...
  async function handleAdminCreateEmployee() {
    ...
    await supabase.from("employees").upsert({id:empId,full_name:fullName,store_id:storeId,store_name:store.name,position,email,username:finalUsername,password_hash:null});
  ...
  async function handleCheckOut() {
    ...
    navigator.geolocation.getCurrentPosition(
      async(pos)=>{
        ...
        const dist=haversine(latitude,longitude,store.lat,store.lng);
        const isDemoMode=BYPASS_USERS.includes(currentUser.username);
        if(!isDemoMode&&dist>RADIUS_METERS){setCheckState(s=>({...s,searching:false,result:{error:`Estás a ${Math.round(dist)}m de la tienda.`}}));return;}
        const today=new Date();today.setHours(0,0,0,0);
        const now=new Date();
        const rec={id:Date.now().toString(),employee_id:currentUser.employeeId,employee_name:currentUser.fullName,store_id:currentUser.storeId,store_name:store.name,timestamp:now.toISOString(),late_minutes:0,shift:"fuera_horario",note:"",type:"salida",distance:Math.round(dist)};
        await supabase.from("records").insert(rec);
  ...
  async function handleSubmitCut() {
    ...
    setSubmittingCut(true); setCutError("");
    ...
    if(totalEgresos>totalCorte){
      setCutError(`🚨 Los egresos (${formatCurrency(totalEgresos)}) no pueden superar el corte (${formatCurrency(totalCorte)}).`);
      setSubmittingCut(false); return;
    }
  ...
  const now=new Date(),period=getCurrentPeriod(),al=alertLevel(stats.quinceMinutes);
      ...
      <div className="content">
        ...
        {authMode==="login"&&(
          <div className="card">
            ...
            <label>Usuario</label>
            <input type="text" placeholder="ej. ana.garcia" value={loginForm.username} onChange={e=>setLoginForm(f=>({...f,username:e.target.value}))} onKeyDown={e=>{if(e.key==="Enter")handleLogin();}} />
            <label>Contraseña</label>
            <input type="password" placeholder="••••••" value={loginForm.password} onChange={e=>setLoginForm(f=>({...f,password:e.target.value}))} onKeyDown={e=>{if(e.key==="Enter")handleLogin();}} />
            {authError&&<div className="info-box info-red">{authError}</div>}
            <button className="btn btn-primary" onClick={handleLogin} disabled={!loginForm.username||!loginForm.password||authLoading}>{authLoading?"Verificando...":"Entrar →"}</button>
            <button className="btn btn-secondary" onClick={()=>{setAuthMode("setup");setAuthError("");}}>Primera vez — crear contraseña</button>
          </div>
        )}
        {authMode==="setup"&&(
          <div className="card">
            ...
            {authError&&<div className="info-box info-red">{authError}</div>}
            <button className="btn btn-primary" onClick={handleSetupPassword} disabled={!setupForm.username||!setupForm.password||!setupForm.confirmPassword||authLoading}>{authLoading?"Creando...":"Crear contraseña y entrar →"}</button>
    ...
    <div className="app">
      ...
      <div className="content">
        <div className="card" style={{textAlign:"center"}}>
          ...
          {isCajero!==null&&(
            <button className="btn btn-primary" style={{marginTop:20}} onClick={()=>{if(isCajero)setCheckoutStep("form");else setCheckoutStep("done");}}>Continuar →</button>
          )}
        </div>
      </div>
    </div></>
    ...
    <div className="app">
      ...
      <div className="content">
        ...
        <div className="card">
          ...
          <div className="info-box info-amber"><strong>Autorizados:</strong> café, leche, hielo, artículos de operación, propinas. Todo lo demás requiere autorización.</div>
          {checkoutForm.gastos.map((g,i)=>{
            const approved=g.concepto?isApprovedExpense(g.concepto):null;
            return(
              <div key={i}>
                <div className="expense-row">
                  <input type="text" placeholder="Concepto" value={g.concepto} onChange={e=>{updateGasto(i,"concepto",e.target.value);setCutError("");}} style={{marginBottom:0,borderColor:g.concepto?(approved?p.green:p.red):p.foam}} />
                  <input type="number" placeholder="$" value={g.monto} onChange={e=>{updateGasto(i,"monto",e.target.value);setCutError("");}} style={{marginBottom:0,width:90}} />
                  {checkoutForm.gastos.length>1&&<button className="remove-btn" onClick={()=>removeGastoRow(i)}>✕</button>}
                </div>
                {g.concepto&&!approved&&<div className="info-box info-red" style={{marginTop:4,marginBottom:8,fontSize:12}}>⚠️ Requiere autorización.</div>}
              </div>
            );
          })}
          <button className="btn btn-secondary" onClick={addGastoRow} style={{marginTop:0}}>+ Agregar egreso</button>
        </div>
        {isOverspent&&<div className="redFlag"><div style={{fontWeight:700,color:p.red,fontSize:15,marginBottom:6}}>🚨 Alerta</div><div style={{fontSize:13,color:p.red}}>Los egresos superan el corte.</div></div>}
        {cutError&&<div className="redFlag"><div style={{fontSize:13,color:p.red}}>{cutError}</div></div>}
        <div className="card">
          <div className="card-title">📝 Notas</div>
          <textarea placeholder="Incidencias, observaciones..." value={checkoutForm.notas} onChange={e=>setCheckoutForm(f=>({...f,notas:e.target.value}))} />
        </div>
        <button className="btn btn-primary" onClick={handleSubmitCut} disabled={(!checkoutForm.efectivo&&!checkoutForm.tarjeta)||isOverspent||submittingCut}>{submittingCut?"⏳ Guardando...":"✅ Registrar corte"}</button>
    ...
    <div className="app">
      ...
      <div className="content">
        <div className="card" style={{textAlign:"center"}}>
          ...
          <div style={{fontSize:13,color:p.gray,marginBottom:16}}>{isCajero?"Tu salida y corte quedaron registrados.":"Tu salida quedó registrada."}</div>
        </div>
        {tipsMsg&&(<div className="tips-card" style={{background:tipsMsg.bg}}><div style={{fontSize:28,marginBottom:8}}>{tipsMsg.emoji}</div><div style={{fontWeight:500,fontSize:14,color:tipsMsg.color,marginBottom:6}}>{tipsMsg.msg}</div><div style={{fontSize:13,color:tipsMsg.color,opacity:0.85}}>{tipsMsg.sub}</div></div>)}
  ...
  return(
    ...
    <div className="app">
      ...
      <div className="content">
        ...
        {/* ── CHECK ── */}
        {tab==="check"&&(<>
          {period&&<div className="info-box info-green"><strong>Turno activo:</strong> {SCHEDULES[period.key].label}</div>}
          ...
          {checkState.result?.success?(
            <div className="card" style={{textAlign:"center"}}>
              ...
              <div style={{fontFamily:"'Playfair Display',serif",fontSize:18,color:p.coffee,marginBottom:6}}>{checkState.result.rec.late_minutes>0?`Retardo — ${formatMin(checkState.result.rec.late_minutes)}`:"¡Entrada registrada!"}</div>
              <div style={{fontSize:13,color:p.gray,marginBottom:16}}>{new Date(checkState.result.rec.timestamp).toLocaleTimeString("es-MX",{hour:"2-digit",minute:"2-digit"})} · {checkState.result.rec.store_name}</div>
            ...
            <div className="card">
              ...
              <button className="btn btn-check" disabled={checkState.searching} onClick={handleCheckIn}>{checkState.searching&&checkState.mode==="entrada"?"Verificando...":"📍 Check in — Entrada"}</button>
              <button className="btn btn-checkout" disabled={checkState.searching} onClick={()=>{setCheckState(s=>({...s,mode:"salida"}));handleCheckOut();}}>{checkState.searching&&checkState.mode==="salida"?"Verificando...":"🚪 Check out — Salida"}</button>
              <div style={{fontSize:12,color:p.gray,textAlign:"center",marginTop:10}}>Debes estar dentro de {RADIUS_METERS}m de tu tienda</div>
            </div>
          )}
          {tipsMsg&&(<div className="tips-card" style={{background:tipsMsg.bg}}><div style={{fontSize:24,marginBottom:6}}>{tipsMsg.emoji}</div><div style={{fontWeight:500,fontSize:13,color:tipsMsg.color,marginBottom:4}}>{tipsMsg.msg}</div><div style={{fontSize:12,color:tipsMsg.color,opacity:0.85}}>{tipsMsg.sub}</div></div>)}
          {myRecords.length>0&&(
            <div className="card">
              ...
              {myRecords.slice(0,5).map(r=>(
                <div key={r.id} className="rec-row">
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div><div className="rec-name">{r.store_name}</div><div className="rec-meta">{new Date(r.timestamp).toLocaleDateString("es-MX",{weekday:"short",day:"numeric",month:"short"})} · {new Date(r.timestamp).toLocaleTimeString("es-MX",{hour:"2-digit",minute:"2-digit"})}</div></div>
                    <span className={`badge ${r.type==="salida"?"badge-blue":r.late_minutes>0?"badge-red":"badge-green"}`}>{r.type==="salida"?"Salida":r.late_minutes>0?`+${formatMin(r.late_minutes)}`:"Puntual"}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {tab==="pedidos"&&(<>
          <div className="tab-row">
            {[["levantar","📋 Levantar"],["historial","📜 Historial"]].map(([k,l])=>(
              <button key={k} className="tab-btn" onClick={()=>setOrderTab(k)}
                style={{background:orderTab===k?p.coffee:"white",color:orderTab===k?p.cream:p.coffee,border:`1.5px solid ${orderTab===k?p.coffee:p.foam}`,fontSize:12}}>
                {l}
              </button>
            ))}
          </div>
          ...
          {orderTab==="levantar"&&(<>
            {orderResult?(
              <div className="card" style={{textAlign:"center"}}>
                <div style={{fontSize:48,margin:"8px 0 12px"}}>✅</div>
                <div style={{fontFamily:"'Playfair Display',serif",fontSize:18,color:p.coffee,marginBottom:8}}>Pedido enviado a revisión</div>
                <div style={{fontSize:13,color:p.gray,marginBottom:16,lineHeight:1.6}}>
                  {orderResult.items.reduce((a,i)=>a+i.qty_ordered,0)} piezas · ${Math.round(orderResult.totalCost).toLocaleString("es-MX")} MXN estimado<br/>
                  Será revisado por Luis, Victoria o Fernanda.
                </div>
                <button className="btn btn-primary" onClick={()=>{setOrderResult(null);setOrderTab("historial");}}>Ver mis pedidos →</button>
                <button className="btn btn-secondary" onClick={()=>setOrderResult(null)}>Nuevo pedido</button>
              </div>
            ):(<>
              {(isWeekendBoost||eventBoost)&&<div className="info-box info-blue">⭐ <strong>+30% activo</strong> — {eventBoost?`Evento: ${eventName||"especial"}`:"Fin de semana"}. Los máximos ya incluyen el incremento.</div>}
              {hasAnomalies()&&<div className="info-box info-amber">⚠️ Uno o más productos tienen stock igual o mayor al máximo. El admin lo revisará.</div>}
              {new Date().getHours()>=ORDER_DEADLINE_HOUR&&<div className="info-box info-red">🔔 Ya pasaron las 6pm — envía el pedido lo antes posible.</div>}

              <div className="card">
                <div className="card-title">📋 Stock actual → pedido</div>
                <div style={{fontSize:12,color:p.gray,marginBottom:14,lineHeight:1.5}}>Captura cuántas piezas tienes. El pedido se calcula solo (máximo − stock).</div>
                {["Tradicionales","Croissant","Croissant Roll","Nidos","Galletas","Especiales"].map(cat=>{
                  const prods=KAIZEEN_PRODUCTS.filter(x=>x.category===cat);
                  return(
                    <div key={cat} style={{marginBottom:16}}>
                      <div className="section-title">{cat}</div>
                      {prods.map(prod=>{
                        const max=getMax(prod.id);
                        const stock=stockInputs[prod.id];
                        const order=getOrder(prod.id);
                        const over=isOverstocked(prod.id);
                        const pct=stock!==undefined&&!isNaN(parseInt(stock))?Math.min(100,Math.round(parseInt(stock)/Math.max(max,1)*100)):0;
                        return(
                          <div key={prod.id} style={{border:`1.5px solid ${over?p.amber:p.foam}`,borderRadius:10,padding:"12px",marginBottom:8,background:over?"#FFF8E1":"white"}}>
                            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                              <span style={{fontWeight:500,fontSize:14,color:p.espresso}}>{prod.name}</span>
                              <span style={{fontSize:11,background:p.foam,padding:"2px 8px",borderRadius:20,color:p.gray}}>Máx: {max} · ${prod.price}</span>
                            </div>
                            <div style={{display:"flex",alignItems:"center",gap:10}}>
                              <div style={{width:80}}>
                                <label style={{marginBottom:3}}>Stock</label>
                                <input type="number" min="0" max="99" inputMode="numeric" placeholder="0"
                                  value={stock!==undefined?stock:""}
                                  onChange={e=>setStockInputs(s=>({...s,[prod.id]:e.target.value}))}
                                  style={{marginBottom:0,textAlign:"center",padding:"8px 6px",fontSize:16}} />
                              </div>
                              <div style={{flex:1}}>
                                <div style={{height:4,background:p.foam,borderRadius:2,marginBottom:6,overflow:"hidden"}}>
                                  <div style={{height:"100%",width:`${pct}%`,background:over?p.amber:p.caramel,borderRadius:2,transition:"width 0.3s"}} />
                                </div>
                                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                                  <span style={{fontSize:11,color:p.gray}}>prom 2/día</span>
                                  <span style={{fontSize:20,fontWeight:700,color:order===0?p.gray:p.coffee,opacity:order===null?0.3:1}}>
                                    {order!==null?order:"—"}
                                  </span>
                                </div>
                                {over&&<div style={{fontSize:11,color:p.amber,marginTop:2}}>⚠ Stock alto</div>}
                              </div>
                            </div>
                          </div>
                        );
              ...
              {getTotalPiezas()>0&&(
                <div className="card">
                  ...
                  {KAIZEEN_PRODUCTS.filter(prod=>(getOrder(prod.id)||0)>0).map(prod=>(
                    <div key={prod.id} className="stat-row">
                      <span className="stat-label">{prod.name}</span>
                      <span className="stat-value">{getOrder(prod.id)} pz</span>
                    </div>
                  ))}
                  <div className="stat-row" style={{borderTop:`2px solid ${p.foam}`,marginTop:4}}>
                    <span className="stat-label">Total piezas</span><span className="stat-value">{getTotalPiezas()}</span>
                  </div>
                  <div className="stat-row">
                    <span className="stat-label">Costo estimado + envío</span>
                    <span className="stat-value" style={{color:p.coffee}}>${Math.round(getTotalCost()).toLocaleString("es-MX")}</span>
                  </div>
                </div>
              )}

              <button className="btn btn-primary" style={{background:p.caramel}} onClick={handleSubmitOrder}
                disabled={submittingOrder||getTotalPiezas()===0||Object.keys(stockInputs).length===0}>
                {submittingOrder?"Enviando...":"📤 Enviar pedido a revisión"}
              </button>
            </>)}
          </>)}
          ...
          {orderTab==="historial"&&(
            <div className="card">
              ...
              {!myOrders.length&&<div style={{textAlign:"center",color:p.gray,fontSize:13,padding:"20px 0"}}>No hay pedidos aún</div>}
              {myOrders.map(o=>{
                const statusColor={pending:p.amber,approved:p.green,rejected:p.red,sent:p.blue,draft:p.gray}[o.status]||p.gray;
                const statusLabel={pending:"En revisión",approved:"Aprobado",rejected:"Rechazado",sent:"Enviado a Kaizeen",draft:"Borrador"}[o.status]||o.status;
                return(
                  <div key={o.id} className="rec-row">
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                      <div>
                        <div className="rec-name">{new Date(o.submitted_at).toLocaleDateString("es-MX",{weekday:"short",day:"numeric",month:"short"})}</div>
                        <div className="rec-meta">{o.order_items?.reduce((a,i)=>a+i.qty_ordered,0)} piezas · ${Math.round(o.total_cost||0).toLocaleString("es-MX")} MXN</div>
                        {o.reviewer_note&&<div style={{fontSize:11,color:o.status==="rejected"?p.red:p.green,marginTop:3}}>💬 {o.reviewer_note}</div>}
                      </div>
                      <span className="badge" style={{background:statusColor+"22",color:statusColor,whiteSpace:"nowrap"}}>{statusLabel}</span>
                    </div>
                  </div>
                );
              })}
        ...
        {tab==="profile"&&currentUser&&(<>
          <div className="card">
            <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:4}}>
              ...
              <div>
                ...
                <div style={{fontSize:13,color:p.gray}}>{currentUser.storeName} · {currentUser.position}</div>
          ...
          <button className="btn btn-secondary" onClick={()=>{removeLS("ccc_user_v2");setCurrentUser(null);setScreen("auth");setLoginForm({username:"",password:""});}}>Cerrar sesión</button>
        </>)}
        ...
        {tab==="admin"&&adminUnlocked&&(<>
          <div style={{display:"flex",justifyContent:"flex-end",marginBottom:12}}>
            <button onClick={()=>{loadAdminData();loadAllOrders();}} style={{fontSize:12,background:"none",border:"none",color:p.caramel,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>↻ Actualizar</button>
          </div>

          <div className="tab-row">
            {["hoy","cortes","pedidos","empleados","digest"].map(t=>(
              <button key={t} className="tab-btn" onClick={()=>setAdminTab(t)}
                style={{background:adminTab===t?p.coffee:"white",color:adminTab===t?p.cream:p.coffee,border:`1.5px solid ${adminTab===t?p.coffee:p.foam}`,fontSize:10}}>
                {t==="hoy"?"Hoy":t==="cortes"?"Cortes":t==="pedidos"?"🥐 Pedidos":t==="empleados"?"Empleados":"Digest"}
              </button>
            ))}
          </div>
          ...
          {!loadingAdmin&&adminTab==="hoy"&&(
            <div className="card">
              <div className="card-title">Hoy — {now.toLocaleDateString("es-MX",{weekday:"long",day:"numeric",month:"long"})}</div>
              {STORES.map(store=>{
          ...
          {!loadingAdmin&&adminTab==="cortes"&&(
            <div className="card">
              ...
              {STORES.map(store=>{
                const sc=cuts.filter(c=>c.store_id===store.id);
                      ...
                      <div key={c.id} className="rec-row">
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                          <div>
                            ...
                            <div className="rec-meta">{new Date(c.timestamp).toLocaleDateString("es-MX",{weekday:"short",day:"numeric",month:"short"})}</div>
                            <div className="rec-meta">Efectivo: {formatCurrency(c.efectivo)} · Tarjeta: {formatCurrency(c.tarjeta)}</div>
          ...
          {adminTab==="pedidos"&&(<>
            {/* Boost */}
            <div className="card">
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",paddingBottom:12,borderBottom:`1px solid ${p.foam}`,marginBottom:12}}>
                <div><div style={{fontWeight:500,fontSize:14,color:p.coffee}}>+30% fin de semana</div><div style={{fontSize:12,color:p.gray,marginTop:2}}>Vie / Sáb / Dom</div></div>
                <div style={{width:44,height:24,borderRadius:12,background:isWeekendBoost?p.coffee:p.foam,cursor:"pointer",position:"relative",transition:"background 0.2s"}} onClick={()=>setIsWeekendBoost(v=>!v)}>
                  <div style={{position:"absolute",width:18,height:18,borderRadius:"50%",background:"white",top:3,left:isWeekendBoost?23:3,transition:"left 0.2s"}} />
                </div>
              </div>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:eventBoost?8:0}}>
                <div><div style={{fontWeight:500,fontSize:14,color:p.coffee}}>Evento especial</div><div style={{fontSize:12,color:p.gray,marginTop:2}}>+30% todas las tiendas hoy</div></div>
                <div style={{width:44,height:24,borderRadius:12,background:eventBoost?p.coffee:p.foam,cursor:"pointer",position:"relative",transition:"background 0.2s"}} onClick={()=>setEventBoost(v=>!v)}>
                  <div style={{position:"absolute",width:18,height:18,borderRadius:"50%",background:"white",top:3,left:eventBoost?23:3,transition:"left 0.2s"}} />
                </div>
              </div>
              {eventBoost&&<input type="text" placeholder="Describe el evento..." value={eventName} onChange={e=>setEventName(e.target.value)} style={{marginBottom:0,marginTop:8}} />}
            </div>

            {/* Métricas */}
            <div className="mini-grid">
              <div className="mini-card"><div className="mini-val" style={{color:p.amber}}>{allOrders.filter(o=>o.status==="pending").length}</div><div className="mini-lbl">Por revisar</div></div>
              <div className="mini-card"><div className="mini-val" style={{color:p.green}}>{allOrders.filter(o=>o.status==="approved").length}</div><div className="mini-lbl">Aprobados</div></div>
              <div className="mini-card"><div className="mini-val" style={{color:p.blue}}>{allOrders.filter(o=>o.status==="sent").length}</div><div className="mini-lbl">Enviados</div></div>
            </div>

            {loadingOrders&&<div style={{textAlign:"center",color:p.gray,padding:"20px 0",fontSize:13}}>Cargando pedidos...</div>}

            {/* Pendientes */}
            {allOrders.filter(o=>o.status==="pending").length>0&&(
              <div className="card">
                <div className="card-title">⏳ Pendientes de revisión</div>
                {allOrders.filter(o=>o.status==="pending").map(o=>{
                  const flags=o.order_items?.filter(i=>{ const m=maximos.find(x=>x.product_id===i.product_id); return m&&i.stock_reported>=m.max_qty; })||[];
                  return(
                    <div key={o.id} style={{borderBottom:`1px solid ${p.foam}`,paddingBottom:14,marginBottom:14}}>
                      <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                        <div>
                          <div style={{fontWeight:500,fontSize:14}}>{o.store_name}</div>
                          <div style={{fontSize:12,color:p.gray}}>{new Date(o.submitted_at).toLocaleString("es-MX",{day:"numeric",month:"short",hour:"2-digit",minute:"2-digit"})} · {o.submitted_by}</div>
                          <div style={{fontSize:12,color:p.gray}}>{o.order_items?.reduce((a,i)=>a+i.qty_ordered,0)} piezas · ${Math.round(o.total_cost||0).toLocaleString("es-MX")} MXN</div>
                        </div>
                        {flags.length>0&&<span className="badge badge-amber">⚠ {flags.length}</span>}
                      </div>
                      <div style={{background:p.milk,borderRadius:8,padding:"8px 10px",marginBottom:8}}>
                        {o.order_items?.filter(i=>i.qty_ordered>0).map(i=>{
                          const prod=KAIZEEN_PRODUCTS.find(x=>x.id===i.product_id);
                          const m=maximos.find(x=>x.product_id===i.product_id);
                          const isFlag=m&&i.stock_reported>=m.max_qty;
                          return(
                            <div key={i.id} style={{display:"flex",justifyContent:"space-between",fontSize:12,padding:"3px 0",color:isFlag?"#E65100":p.espresso}}>
                              <span>{prod?.name||i.product_id}{isFlag?" ⚠":""}</span>
                              <span style={{fontWeight:500}}>{i.qty_ordered} pz</span>
                            </div>
                          );
                        })}
                      </div>
                      {flags.length>0&&<div className="info-box info-amber" style={{marginBottom:8,fontSize:12}}>Stock reportado ≥ máximo en {flags.length} producto(s). Nota obligatoria.</div>}
                      <textarea placeholder={flags.length>0?"Nota obligatoria (hay alertas)...":"Nota opcional..."} value={reviewNote[o.id]||""} onChange={e=>setReviewNote(n=>({...n,[o.id]:e.target.value}))} style={{minHeight:60,marginBottom:8,fontSize:13}} />
                      <div style={{display:"flex",gap:8}}>
                        <button className="btn btn-primary" style={{fontSize:13,padding:"10px"}} onClick={()=>handleApproveOrder(o.id)}>✅ Aprobar</button>
                        <button className="btn btn-secondary" style={{fontSize:13,padding:"10px",marginTop:0,color:p.red,borderColor:p.red}} onClick={()=>handleRejectOrder(o.id)}>✕ Rechazar</button>
                      </div>
                    </div>
                  );
                })}
            ...
            {allOrders.filter(o=>o.status==="approved").length>0&&(
              <div className="card">
                <div className="card-title">📦 Listo para Kaizeen</div>
                <div style={{fontSize:13,color:p.gray,marginBottom:12}}>{allOrders.filter(o=>o.status==="approved").length} pedido(s) aprobado(s) de {STORES.length} tiendas.</div>
                <button className="btn btn-primary" style={{background:p.green}} onClick={sendToKaizen}>💬 Enviar pedido consolidado por WhatsApp</button>
              </div>
            )}

            {/* Sin pendientes */}
            {allOrders.filter(o=>o.status==="pending").length===0&&allOrders.filter(o=>o.status==="approved").length===0&&(
              <div className="card" style={{textAlign:"center"}}>
                <div style={{fontSize:36,marginBottom:10}}>🥐</div>
                <div style={{fontSize:13,color:p.gray}}>No hay pedidos pendientes de revisión.</div>
              </div>
            )}

            {/* Historial */}
            {allOrders.filter(o=>["approved","rejected","sent"].includes(o.status)).length>0&&(
              <div className="card">
                <div className="card-title">Historial</div>
                {allOrders.filter(o=>["approved","rejected","sent"].includes(o.status)).slice(0,15).map(o=>{
                  const statusColor={approved:p.green,rejected:p.red,sent:p.blue}[o.status]||p.gray;
                  const statusLabel={approved:"Aprobado",rejected:"Rechazado",sent:"Enviado"}[o.status]||o.status;
                  return(
                    <div key={o.id} className="rec-row">
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                        <div>
                          <div className="rec-name">{o.store_name}</div>
                          <div className="rec-meta">{new Date(o.submitted_at).toLocaleDateString("es-MX",{weekday:"short",day:"numeric",month:"short"})} · {o.order_items?.reduce((a,i)=>a+i.qty_ordered,0)} pz · ${Math.round(o.total_cost||0).toLocaleString("es-MX")}</div>
                          {o.reviewer_note&&<div style={{fontSize:11,color:p.gray,marginTop:2}}>💬 {o.reviewer_note}</div>}
                        </div>
            ...
            <button className="btn btn-secondary" onClick={loadAllOrders}>↻ Actualizar pedidos</button>
          </>)}
          ...
          {!loadingAdmin&&adminTab==="empleados"&&(<>
            <div className="card">
              ...
              <button className="btn btn-primary" onClick={handleAdminCreateEmployee} disabled={!adminCreateForm.fullName||!adminCreateForm.storeId||!adminCreateForm.position||adminCreating}>
                      ...
                      <div key={e.id} className="rec-row">
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                          <div>
                            ...
                            <div className="rec-meta" style={{color:e.password_hash?p.green:p.amber}}>{e.password_hash?"✓ Cuenta activa":"⏳ Pendiente de activar"}</div>
                          </div>
                        </div>
                      </div>
                    ))}
      ...
      <nav className="nav">
        {[
          {key:"check",  icon:"📍",label:"Checar"},
          {key:"pedidos",icon:"🥐",label:"Pedido"},
