...
export default function App() {
  ...
  const [checkState, setCheckState] = useState({ storeId: currentUser?.storeId || "", employeeId: currentUser?.employeeId || "", searching: false, result: null });
  ...
  useEffect(() => {
    const query = checkState.storeId
      ? supabase.from("employees").select("*").eq("store_id", checkState.storeId)
      : supabase.from("employees").select("*");
    query.then(({ data }) => { if (data) setCheckEmployees(data); });
  }, [checkState.storeId]);
        ...
        <div className="content">
            ...
            {checkState.result?.success ? (
              <div className="card" style={{ textAlign: "center" }}>
                ...
                <div style={{ fontSize: 13, color: p.gray, marginBottom: 16 }}>
                  {new Date(checkState.result.rec.timestamp).toLocaleTimeString("es-MX", { hour: "2-digit", minute: "2-digit" })} · {checkState.result.rec.store_name} · {checkState.result.rec.distance}m
                </div>
                ...
                <button className="btn btn-secondary" onClick={() => setCheckState(s => ({ ...s, result: null }))}>Nueva entrada</button>
              </div>
            ) : (
              <div className="card">
                ...
                {myRecords.slice(0, 5).map(r => (
                  <div key={r.id} className="rec-row">
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      {r.late_minutes > 0 ? <span className="badge badge-red">+{formatMin(r.late_minutes)}</span> : <span className="badge badge-green">Puntual</span>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          {tab === "profile" && currentUser && (<>
            ...
            <div className="card">
              ...
              {[
                { dot: p.green, label: "0–14 min", badge: "badge-green", text: "Puntual" },
                { dot: p.amber, label: "15–59 min tarde", badge: "badge-amber", text: "Retardo" },
                { dot: p.red, label: "60+ min acum./quincena", badge: "badge-red", text: "Descuento posible" },
                { dot: p.red, label: "120+ min acum./quincena", badge: "badge-red", text: "Descuento de día" },
              ].map((row, i) => (
                <div key={i} className="stat-row">
                  <div style={{ display: "flex", alignItems: "center" }}><span className="dot" style={{ background: row.dot }} /><span className="stat-label">{row.label}</span></div>
          ...
          {tab === "admin" && adminUnlocked && (<>
            ...
            {loadingAdmin && <div style={{ textAlign: "center", padding: "40px 0", color: p.gray }}>Cargando datos...</div>}

            {!loadingAdmin && adminTab === "hoy" && (
              <div className="card">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                  <div className="card-title" style={{ margin: 0 }}>Registros de hoy</div>
                  <button onClick={loadAdminData} style={{ fontSize: 12, background: "none", border: "none", color: p.caramel, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>↻ Actualizar</button>
                </div>
                {STORES.map(store => {
                          ...
                          <div key={e.id} className="rec-row">
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                              <div>
                                ...
                                <div className="rec-meta">{e.id} · {e.position}</div>
                              </div>
            ...
            {!loadingAdmin && adminTab === "digest" && (
              <div className="card">
                ...
                <div style={{ fontSize: 13, color: p.gray, marginBottom: 16, lineHeight: 1.7 }}>Envía resumen por tienda a los correos admin.</div>
                ...
                {digestSent === "error" && <div className="info-box info-red">Error al enviar. Verifica Resend.</div>}
                <div style={{ marginTop: 12 }}>
                  <div className="section-title" style={{ marginBottom: 12 }}>Resumen por tienda</div>
                  {STORES.map(store => {
                    const emps = employees.filter(e => e.store_id === store.id);
                    if (!emps.length) return null;
                    const todayRecs = records.filter(r => r.store_id === store.id && new Date(r.timestamp).toDateString() === now.toDateString());
                    return (
                      <div key={store.id} style={{ marginBottom: 14 }}>
                        ...
                        <div className="mini-grid">
                          ...
                          <div className="mini-card"><div className="mini-val">{todayRecs.filter(r => r.late_minutes === 0).length}</div><div className="mini-lbl">Puntual</div></div>
                          <div className="mini-card"><div className="mini-val">{todayRecs.filter(r => r.late_minutes > 0).length}</div><div className="mini-lbl">Retardos</div></div>
                        </div>
                      </div>
                    );
                  })}
